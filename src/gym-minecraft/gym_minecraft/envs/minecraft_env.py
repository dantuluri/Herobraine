...
class MinecraftEnv(gym.Env):
    ...
    def _take_action(self, actions,direct_cmd=False):
        # if there is only one action space, it wasn't wrapped in Tuple
        if direct_cmd:
            logger.debug(actions)
            self.agent_host.sendCommand(actions)
        else:
            if len(self.action_spaces) == 1:
                actions = [actions]
    ...
    def _step(self, action, direct_cmd=True):
        # take the action only if mission is still running
        world_state = self.agent_host.peekWorldState()
        if world_state.is_mission_running:
            # take action
            self._take_action(action,direct_cmd)
